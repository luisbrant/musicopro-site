...
const getSavedTx = () => localStorage.getItem('musicopro_tx') || '';
const setSavedTx = (tx: string) => localStorage.setItem('musicopro_tx', tx);
...
export default function AppOnly() {
  ...
  const validate = async () => {
    ...
    if (!normalizedEmail || !normalizedTx.startsWith('HP')) {
      setStatus('error');
      setMsg('Digite o e-mail e o código da transação (ex: HP...).');
      emailRef.current?.focus();
      return;
    }

    try {
      setStatus('checking'); setMsg('Verificando...');
      setSavedEmail(normalizedEmail);
      setSavedTx(normalizedTx);
      ...
      if (ok) {
        ...
        setStatus('inactive'); setMsg('Dados não econtrados. Verifique ou compre uma licença.');
      }
    } catch (e) {
  ...
  useEffect(() => {
    const savedEmail = getSavedEmail().trim().toLowerCase();
    const savedTx = getSavedTx().trim().toUpperCase();
    if (savedEmail) setEmail(savedEmail);
    if (savedTx) setTransaction(savedTx);
  }, []);
